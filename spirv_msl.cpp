...
uint32_t CompilerMSL::get_automatic_builtin_input_location(spvc::BuiltIn builtin) const
{
...
uint32_t CompilerMSL::get_automatic_builtin_output_location(spvc::BuiltIn builtin) const
{
...
bool CompilerMSL::builtin_translates_to_nonarray(spvc::BuiltIn builtin) const
{
...
void CompilerMSL::build_implicit_builtins()
{
	    ...
	    force_frag_depth_passthrough || need_point_size || is_mesh_shader())
	{
		...
		if (!has_helper_invocation && needs_helper_invocation)
		{
			...
			bool_type_ptr_in.op = spvc::OpTypePointer;
		...
		if (!has_frag_depth && force_frag_depth_passthrough)
		{
			...
			float_type_ptr_in.op = spvc::OpTypePointer;
	...
	if (get_execution_model() == spvc::ExecutionModelTaskEXT)
	{
		uint32_t offset = ir.increase_bound_by(3);
		uint32_t type_id = offset;
		uint32_t type_ptr_id = offset + 1;
		...
		mesh_grid_type_ptr.op = spvc::OpTypePointer;
...
void CompilerMSL::ensure_builtin(spvc::StorageClass storage, spvc::BuiltIn builtin)
{
...
bool CompilerMSL::add_component_variable_to_interface_block(spvc::StorageClass storage, const std::string &ib_var_ref,
                                                            SPIRVariable &var,
                                                            const SPIRType &type,
...
bool CompilerMSL::variable_storage_requires_stage_io(spvc::StorageClass storage) const
{
...
uint32_t CompilerMSL::ensure_correct_builtin_type(uint32_t type_id, BuiltIn builtin)
{
	     ...
	     pointee_type.basetype != SPIRType::UInt))
	{
		...
		ptr_type.op = spvc::OpTypePointer;
		ptr_type.pointer = true;
		ptr_type.pointer_depth++;
...
void CompilerMSL::emit_custom_templates()
{
	...
	for (const auto &spv_func : spv_function_implementations)
	{
		...
		case SPVFuncImplUnsafeArray:
			...
			statement("return elements[pos];");
			end_scope();
			if (get_execution_model() == spvc::ExecutionModelMeshEXT ||
			    get_execution_model() == spvc::ExecutionModelTaskEXT)
			{
				statement("");
...
bool CompilerMSL::prepare_access_chain_for_scalar_access(std::string &expr, const SPIRType &type,
                                                         spvc::StorageClass storage, bool &is_packed)
{
...
uint32_t CompilerMSL::get_physical_tess_level_array_size(spvc::BuiltIn builtin) const
{
	if (is_tessellating_triangles())
...
uint32_t CompilerMSL::get_or_allocate_builtin_input_member_location(spvc::BuiltIn builtin,
...
uint32_t CompilerMSL::get_or_allocate_builtin_output_member_location(spvc::BuiltIn builtin,
                                                                     uint32_t type_id, uint32_t index,
                                                                     uint32_t *comp)
{
	uint32_t loc = get_member_location(type_id, index, comp);
...
{
	return get_execution_model() == spvc::ExecutionModelMeshEXT;
}
		...
		case SPIRType::Struct:
		{
			...
			else if (!type.array.empty())
			{
				...
				if (is_var_runtime_size_array(var))
				{
					...
					if ((var.storage == spvc::StorageClassStorageBuffer || ssbo) &&
...
bool CompilerMSL::variable_decl_is_remapped_storage(const SPIRVariable &variable, spvc::StorageClass storage) const
{
...
bool CompilerMSL::SampledImageScanner::handle(spvc::Op opcode, const uint32_t *args, uint32_t length)
{
	switch (opcode)
	{
	case OpLoad:
