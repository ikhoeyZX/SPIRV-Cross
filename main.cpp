...
static void print_resources(const Compiler &compiler, spvc::StorageClass storage,
                            const SmallVector<BuiltInResource> &resources)
{
	...
	for (auto &res : resources)
	{
		...
		switch (res.builtin)
		{
		case spvc::BuiltInPosition: builtin_str = "Position"; break;
		case spvc::BuiltInPointSize: builtin_str = "PointSize"; break;
		case spvc::BuiltInCullDistance: builtin_str = "CullDistance"; break;
		case spvc::BuiltInClipDistance: builtin_str = "ClipDistance"; break;
		case spvc::BuiltInTessLevelInner: builtin_str = "TessLevelInner"; break;
		case spvc::BuiltInTessLevelOuter: builtin_str = "TessLevelOuter"; break;
...
static const char *execution_model_to_str(spvc::ExecutionModel model)
{
	switch (model)
	{
	case spvc::ExecutionModelVertex:
		return "vertex";
	case spvc::ExecutionModelTessellationControl:
...
#define CHECK_MODE(m)                  \
	...
	print_resources(compiler, spvc::StorageClassInput, res.builtin_inputs);
	print_resources(compiler, spvc::StorageClassOutput, res.builtin_outputs);
}
...
static ExecutionModel stage_to_execution_model(const std::string &stage)
{
	if (stage == "vert")
		...
		return spvc::ExecutionModelMeshEXT;
	else if (stage == "task")
		return spvc::ExecutionModelTaskEXT;
	else
