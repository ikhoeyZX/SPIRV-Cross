...
{
	std::string name;
	spvc::ExecutionModel execution_model;
};
...
public:
	...
	// I.e. (1ull << spvc::DecorationFoo) | (1ull << spvc::DecorationBar)
	...
	// If the decoration is a boolean (i.e. spvc::DecorationNonWritable),
	// 1 will be returned.
	...
	spvc::StorageClass get_storage_class(VariableID id) const;
	...
	void set_entry_point(const std::string &entry, spvc::ExecutionModel execution_model);
	...
	void rename_entry_point(const std::string &old_name, const std::string &new_name,
	                        spvc::ExecutionModel execution_model);
	const SPIREntryPoint &get_entry_point(const std::string &name, spvc::ExecutionModel execution_model) const;
	SPIREntryPoint &get_entry_point(const std::string &name, spvc::ExecutionModel execution_model);
	const std::string &get_cleansed_entry_point_name(const std::string &name,
	                                                 spvc::ExecutionModel execution_model) const;

	// Traverses all reachable opcodes and sets active_builtins to a bitmask of all builtin variables which are accessed in the shader.
	void update_active_builtins();
	bool has_active_builtin(spvc::BuiltIn builtin, spvc::StorageClass storage) const;

	// Query and modify OpExecutionMode.
	const Bitset &get_execution_mode_bitset() const;

	void unset_execution_mode(spvc::ExecutionMode mode);
	void set_execution_mode(spvc::ExecutionMode mode, uint32_t arg0 = 0, uint32_t arg1 = 0, uint32_t arg2 = 0);
	...
	uint32_t get_execution_mode_argument(spvc::ExecutionMode mode, uint32_t index = 0) const;
	spvc::ExecutionModel get_execution_model() const;
	...
	bool get_binary_offset_for_decoration(VariableID id, spvc::Decoration decoration, uint32_t &word_offset) const;
	...
	const SmallVector<spvc::Capability> &get_declared_capabilities() const;
...
protected:
	...
	static bool is_tessellation_shader(spvc::ExecutionModel model);
	...
	bool is_member_builtin(const SPIRType &type, uint32_t index, spvc::BuiltIn *builtin) const;
	...
	struct OpcodeHandler
	{
		...
		virtual bool handle(spvc::Op opcode, const uint32_t *args, uint32_t length) = 0;
	...
	struct ActiveBuiltinHandler : OpcodeHandler
	{
		...
		void handle_builtin(const SPIRType &type, spvc::BuiltIn builtin, const Bitset &decoration_flags);
	...
	struct CFGBuilder : OpcodeHandler
	{
		...
		bool handle(spvc::Op op, const uint32_t *args, uint32_t length) override;
		Compiler &compiler;
		std::unordered_map<uint32_t, std::unique_ptr<CFG>> function_cfgs;
	};
	...
	bool instruction_to_result_type(uint32_t &result_type, uint32_t &result_id, spvc::Op op, const uint32_t *args,
	                                uint32_t length);

	Bitset combined_decoration_for_member(const SPIRType &type, uint32_t index) const;
	static bool is_desktop_only_format(spvc::ImageFormat format);
