...
std::string CompilerHLSL::builtin_to_glsl(spvc::BuiltIn builtin, spvc::StorageClass storage)
{
	switch (builtin)
	{
	case BuiltInVertexId:
...
void CompilerHLSL::emit_texture_op(const Instruction &i, bool sparse)
{
	...
	if (hlsl_options.shader_model < 40)
	{
		if (dref)
		{
			if (imgtype.image.dim != spvc::Dim1D && imgtype.image.dim != spvc::Dim2D)
			{
				SPIRV_CROSS_THROW(
...
void CompilerHLSL::emit_atomic(const uint32_t *ops, uint32_t length, spvc::Op op)
{
...
bool CompilerHLSL::is_hlsl_force_storage_buffer_as_uav(ID id) const
{
	...
	const uint32_t desc_set = get_decoration(id, spvc::DecorationDescriptorSet);
	const uint32_t binding = get_decoration(id, spvc::DecorationBinding);
