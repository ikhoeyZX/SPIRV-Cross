...
public:
	...
	void mask_stage_output_by_builtin(spvc::BuiltIn builtin);
...
protected:
	...
	static bool is_supported_subgroup_op_in_opengl(spvc::Op op, const uint32_t *ops);
	...
	spvc::Op get_remapped_spirv_op(spvc::Op op) const;
	...
	virtual std::string builtin_to_glsl(spvc::BuiltIn builtin, spvc::StorageClass storage);
	...
	virtual bool variable_decl_is_remapped_storage(const SPIRVariable &var, spvc::StorageClass storage) const;
	...
	virtual bool builtin_translates_to_nonarray(spvc::BuiltIn builtin) const;
	...
	void emit_resources();
	void emit_extension_workarounds(spvc::ExecutionModel model);
	void emit_subgroup_arithmetic_workaround(const std::string &func, spvc::Op op, spvc::GroupOperation group_op);
	void emit_polyfills(uint32_t polyfills, bool relaxed);
	void emit_buffer_block_native(const SPIRVariable &var);
	void emit_buffer_reference_block(uint32_t type_id, bool forward_declaration);
	void emit_buffer_block_legacy(const SPIRVariable &var);
	void emit_buffer_block_flattened(const SPIRVariable &type);
	void fixup_implicit_builtin_block_names(spvc::ExecutionModel model);
	void emit_declared_builtin_block(spvc::StorageClass storage, spvc::ExecutionModel model);
	bool should_force_emit_builtin_block(spvc::StorageClass storage);
	...
	spvc::StorageClass get_expression_effective_storage_class(uint32_t ptr);
	...
	const char *format_to_glsl(spvc::ImageFormat format);
	...
	virtual bool emit_array_copy(const char *expr, uint32_t lhs_id, uint32_t rhs_id,
	                             spvc::StorageClass lhs_storage, spvc::StorageClass rhs_storage);
	...
	bool can_use_io_location(spvc::StorageClass storage, bool block);
	...
	virtual SPIRType::BaseType get_builtin_basetype(spvc::BuiltIn builtin, SPIRType::BaseType default_type);
	...
	bool is_stage_output_builtin_masked(spvc::BuiltIn builtin) const;
