...
static std::pair<spvc::Op, SPIRType::BaseType> pls_format_to_basetype(PlsFormat format)
{
	switch (format)
	{
	default:
	...
	case PlsRGBA8:
	case PlsRG16:
		return std::make_pair(spvc::OpTypeFloat, SPIRType::Float);

	case PlsRGBA8I:
	case PlsRG16I:
		return std::make_pair(spvc::OpTypeInt, SPIRType::Int);

	case PlsRGB10A2UI:
	case PlsRGBA8UI:
	case PlsRG16UI:
	case PlsR32UI:
		return std::make_pair(spvc::OpTypeInt, SPIRType::UInt);
	}
}
...
const char *CompilerGLSL::format_to_glsl(spvc::ImageFormat format)
{
...
void CompilerGLSL::emit_extension_workarounds(spvc::ExecutionModel model)
{
	...
	if (!options.vulkan_semantics)
	{
		...
		{
			if (model == spvc::ExecutionModelGLCompute)
			{
				statement("#ifndef GL_KHR_shader_subgroup_basic");
		...
		auto arithmetic_feature_helper =
		    [&](Supp::Feature feat, std::string func_name, spvc::Op op, spvc::GroupOperation group_op)
		{
                                         ...
                                         bool inside_struct_scope)
{
	...
	else if (c.replicated && type.op != spvc::OpTypeArray)
	{
		if (type.op == spvc::OpTypeMatrix)
		{
	...
	else if (c.columns() == 1 && type.op != spvc::OpTypeCooperativeMatrixKHR)
	{
...
string CompilerGLSL::legacy_tex_op(const std::string &op, const SPIRType &imgtype, uint32_t tex)
{
	...
	if (is_comparison && is_legacy_es())
	{
		...
		if (imgtype.image.dim == spvc::DimCube)
			return "shadowCubeNV";
	}
...
bool CompilerGLSL::is_supported_subgroup_op_in_opengl(spvc::Op op, const uint32_t *ops)
{
	switch (op)
	{
...
string CompilerGLSL::builtin_to_glsl(BuiltIn builtin, StorageClass storage)
{
	...
	case BuiltInInstanceId:
		if (options.vulkan_semantics)
		{
			...
			switch (model)
			{
			case spvc::ExecutionModelIntersectionKHR:
			case spvc::ExecutionModelAnyHitKHR:
			case spvc::ExecutionModelClosestHitKHR:
...
bool CompilerGLSL::prepare_access_chain_for_scalar_access(std::string &, const SPIRType &, spvc::StorageClass, bool &)
{
	return false;
}
...
string CompilerGLSL::build_composite_combiner(uint32_t return_type, const uint32_t *elems, uint32_t length)
{
	...
	bool can_apply_swizzle_opt = type.basetype != SPIRType::Struct && type.array.empty() && type.columns == 1 &&
	                             type.op != spvc::OpTypeCooperativeMatrixKHR;
...
#define GLSL_UFOP(op) emit_unary_func_op(ops[0], ops[1], ops[2], #op)
	...
	case OpCompositeExtract:
	{
		...
		if (composite_type.op == spvc::OpTypeCooperativeMatrixKHR)
			allow_base_expression = false;
	...
	case OpFConvert:
	{
		...
		if ((type.basetype == SPIRType::FloatE4M3 || type.basetype == SPIRType::FloatE5M2) &&
		    has_decoration(id, spvc::DecorationSaturatedToLargestFloat8NormalConversionEXT))
		{
			emit_uninitialized_temporary_expression(result_type, id);
	...
	case OpFragmentMaskFetchAMD:
	{
		...
		if (type.image.dim == spvc::DimSubpassData)
		{
...
#undef GLSL_RAY_QUERY_GET_OP2
	...
	case OpTensorReadARM:
	{
		...
		if (length > 4)
		{
			...
			else if (ops[4] == spvc::TensorOperandsNontemporalARMMask)
				tensor_operands = "gl_TensorOperandsNonTemporalARM";
			else if (ops[4] == spvc::TensorOperandsOutOfBoundsValueARMMask)
				tensor_operands = "gl_TensorOperandsOutOfBoundsValueARM";
			else if (ops[4] == (spvc::TensorOperandsNontemporalARMMask | spvc::TensorOperandsOutOfBoundsValueARMMask))
			...
			if ((ops[4] & spvc::TensorOperandsOutOfBoundsValueARMMask) && length != 6)
		...
		if (length >= 6)
		{
			if ((length > 6) || (ops[4] & spvc::TensorOperandsOutOfBoundsValueARMMask) == 0)
	...
	case OpTensorWriteARM:
	{
		...
		if (length > 3)
		{
			...
			if (ops[3] == 0)
				tensor_operands = "0x0u";
			else if (ops[3] == spvc::TensorOperandsNontemporalARMMask)
				tensor_operands = "gl_TensorOperandsNonTemporalARM";
			else
	...
	case OpCompositeConstructReplicateEXT:
	{
		...
		if (type.op == spvc::OpTypeMatrix || type.op == spvc::OpTypeArray)
		{
			if (type.op == spvc::OpTypeArray && type.array.size() != 1)
			{
				SPIRV_CROSS_THROW(
				    "Multi-dimensional arrays currently not supported for OpCompositeConstructReplicateEXT");
			}
			uint32_t num_elements = type.op == spvc::OpTypeMatrix ? type.columns : type.array[0];
			if (backend.use_initializer_list && type.op == spvc::OpTypeArray)
			{
				rhs += "{";
			}
			else
...
string CompilerGLSL::type_to_glsl(const SPIRType &type, uint32_t id)
{
	...
	if (type.op == spvc::OpTypeCooperativeVectorNV)
	{
	...
	if (coop_type->op == spvc::OpTypeCooperativeMatrixKHR)
	{
		...
		if (const auto *scope = maybe_get<SPIRConstant>(coop_type->ext.cooperative.scope_id))
		{
			if (!scope->specialization)
			{
				...
				if (scope->scalar() == spvc::ScopeSubgroup)
					scope_expr = "gl_ScopeSubgroup";
				else if (scope->scalar() == spvc::ScopeWorkgroup)
...
bool CompilerGLSL::builtin_translates_to_nonarray(spvc::BuiltIn /*builtin*/) const
{
...
void CompilerGLSL::add_function_overload(const SPIRFunction &func)
{
	...
	for (auto &arg : func.arguments)
	{
		...
		if (get<SPIRType>(type_id).op == spvc::OpTypeCooperativeMatrixKHR)
...
bool CompilerGLSL::is_stage_output_builtin_masked(spvc::BuiltIn builtin) const
{
