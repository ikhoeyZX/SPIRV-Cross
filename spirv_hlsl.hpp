...
public:
	...
	bool is_hlsl_resource_binding_used(spvc::ExecutionModel model, uint32_t set, uint32_t binding) const;
...
private:
	...
	void emit_atomic(const uint32_t *ops, uint32_t length, spvc::Op op);
	...
	SPIRType::BaseType get_builtin_basetype(spvc::BuiltIn builtin, SPIRType::BaseType default_type) override;
	...
	std::string to_semantic(uint32_t location, spvc::ExecutionModel em, spvc::StorageClass sc);
